...
def test_rv_oe():
    ...
    oe = rv2elem_parab(gravitational_parameter, position, velocity)
    r, v = elem2rv_parab(gravitational_parameter, oe)
    ...
    velocity = np.sqrt(gravitational_parameter / np.linalg.norm(position)) * np.cross(h, position / np.linalg.norm(
        position))
    oe = rv2elem_parab(gravitational_parameter, position, velocity)
    np.testing.assert_almost_equal(oe.a, np.linalg.norm(position), 8)
    np.testing.assert_almost_equal(oe.e, 0, 8)
    np.testing.assert_almost_equal(oe.i, 0, 8)
    np.testing.assert_almost_equal(oe.f, 0, 8)
