...
def test_pixel_error_uniform_offset(circle_image):
    ...
    # Dim the disk by a known amount: the mean error is that offset spread over the whole frame (the
    # background is identical in both), and the std is the spread of a two-valued error map.
    offset = 20
    ...
    # Masking to the disk removes the perfectly-matching background: the error is then the offset
    # itself, with no spread at all.
    fg = circle_image > 0
...
def test_batch_pixel_error_pools_every_pixel(circle_image):
    ...
    # Two frames with opposite-sign offsets: the pooled mean cancels to zero while MAE does not, and
    # the frame-to-frame scatter reports the disagreement the mean hides.
