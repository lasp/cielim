...
def orbital_elements_to_cartesian(mu: float, elements: ClassicOrbitalElements) -> (np.ndarray, np.ndarray):
    ...
    if np.abs(elements.eccentricity - 1) < circular_threshold and elements.semi_major_axis > 0:
        ...
        direction[0] = (np.cos(elements.ascending_node) * np.cos(elements.argument_periapsis) -
                 np.sin(elements.ascending_node) * np.sin(elements.argument_periapsis) * np.cos(elements.inclination))
        direction[1] = (np.sin(elements.ascending_node) * np.cos(elements.argument_periapsis) +
                 np.cos(elements.ascending_node) * np.sin(elements.argument_periapsis) * np.cos(elements.inclination))
        ...
        position = np.zeros(3)
        position[0] = r * (np.cos(elements.ascending_node) * np.cos(theta) -
                           np.sin(elements.ascending_node) * np.sin(theta) * np.cos(elements.inclination))
        position[1] = r * (np.sin(elements.ascending_node) * np.cos(theta) +
        ...
        velocity = np.zeros(3)
        velocity[0] = -mu / h * (np.cos(elements.ascending_node) *
        ...
        velocity[1] = -mu / h * (np.sin(elements.ascending_node) *
                                 (np.sin(theta) + elements.eccentricity * np.sin(elements.argument_periapsis))
                                 - np.cos(elements.ascending_node) *
                                 (np.cos(theta) + elements.eccentricity * np.cos(elements.argument_periapsis)) *
                                 np.cos(elements.inclination))
        velocity[2] = -mu / h * (-(np.cos(theta) + elements.eccentricity
                                   * np.cos(elements.argument_periapsis)) * np.sin(elements.inclination))
