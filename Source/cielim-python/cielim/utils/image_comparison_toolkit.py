...
# Signed difference heatmap, black at zero so a perfect match is unmistakable. Both halves share the
# inferno ramp, so color encodes the magnitude of the difference and not its sign.
...
# ROI cropping: small faint disks get a large relative pad, large disks a tight one.
PAD_SMALL = 1.4
PAD_LARGE = 0.20
SMALL_THRESHOLD = 50
DISPLAY_SIZE = 300
...
def generate_batch(
    ...
    for mode in modes:
        ...
        for i, (real, gen) in enumerate(full):
            ...
            # Measured on this window, the pixels the row shows, so caption, CSVs and average agree.
            # Masked counts only the target: its zeroed background would drag the mean toward zero.
