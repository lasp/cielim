...
# Page geometry in inches. PAGE_W is the paper's text width, so a full-width figure is built at
# exactly this and needs no scaling when placed; HALF_W is one panel of a two-panel figure.
PAGE_W = 6.5
HALF_W = PAGE_W / 2
SAVE_DPI = 200

# All figure text is BODY_PT, matching the paper's body size, so a figure included at scale 1.0
# needs no rescaling -- scaling a figure scales its text too.
BODY_PT = 10
