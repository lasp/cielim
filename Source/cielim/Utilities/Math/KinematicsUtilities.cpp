...
UE::Math::TQuat<double> MRPtoQuaternion(const FVector3d &v)
{
	...
	return UE::Math::TQuat(2 * v.X / Ps, 2 * v.Y / Ps, 2 * v.Z / Ps, (1 - NormSquared) / Ps);
}
 ...
 */
UE::Math::TQuat<double> RightQuat2LeftQuat(const FQuat &q) { return UE::Math::TQuat<double>(-q.X, q.Y, -q.Z, q.W); }
 ...
 */
UE::Math::TVector<double> Right2LeftVector(const FVector3d &v) { return UE::Math::TVector<double>(v.X, -v.Y, v.Z); }
