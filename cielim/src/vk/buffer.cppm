...
/* Purpose: A buffer represents GPU-visible memory, allocated through VMA. It may optionally be mapped for direct CPU
 * access and/or given a GPU device virtual address so it can be read without descriptor sets. */
...
public:
    ...
    ) -> Result<void>
    {
            ...
            result != VK_SUCCESS)
        {
            error::DetailedError error = {
                .errc = make_error_code(error::VkAllocatorError::BufferCreateError),
        ...
        // Null unless alloc_flags requested mapped memory
        this->mapped_data_ = allocation_info.pMappedData;
    ...
    [[nodiscard]] auto get_mapped_data() const -> void* { return this->mapped_data_; }
